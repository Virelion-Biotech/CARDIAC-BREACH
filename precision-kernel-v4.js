...
(()=>{
  ...
  class FixedLedger{
    constructor(seed){this.seed=seed>>>0;this.random=rng64(this.seed);this.tick=0;this.microstep=0;this.accounts=new Map();this.audit=[]}
    ...
    restore(snapshot){if(!snapshot)return;this.tick=Number(snapshot.tick)||0;this.microstep=Number(snapshot.microstep)||0;this.accounts=new Map(Object.entries(snapshot.accounts||{}).map(([k,v])=>[k,BigInt(v)]));this.audit=Array.isArray(snapshot.audit)?snapshot.audit.slice(-64):[]}
  }
  ...
  function attach(game,snapshot=null){
    ...
    const precision={
      version:4,scale:'1e-18',unit:'Q18',ledger,microstepsPerTurn:4096,
      sync(){mirrorFloatState(game)},
      phase(name,fn){ledger.beginPhase(name);const out=fn();ledger.endPhase();return out},
      ...
      exact(name){return ledger.get(name)},
      snapshot(){return ledger.snapshot()}
    };
    ledger.microstepsPerPhase=4096;
    game.precision=precision;
    if(snapshot)ledger.restore(snapshot);else precision.sync();
    return precision;
  }
