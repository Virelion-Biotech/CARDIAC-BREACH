/* CARDIAC//BREACH v4 — deterministic precision kernel
 * Bounded physiological state uses Q18 fixed-point units; signed accounting
 * uses a separate exact ledger. This is reproducible bookkeeping, not a claim
 * of GPU-like hardware precision.
 */
(()=>{
  'use strict';
  ...
  const toNumber=v=>Number(v)/1e18,q=v=>typeof v==='bigint'?v:fromNumber(v);
  const mul=(a,b)=>(a*b)/SCALE,div=(a,b)=>b===ZERO?ZERO:(a*SCALE)/b;
  function rng64(seed){let state=(BigInt(seed>>>0)<<32n)|BigInt(seed>>>0),mask=0xffffffffffffffffn;return()=>{state=(state+0x9e3779b97f4a7c15n)&mask;let z=state;z=((z^(z>>30n))*0xbf58476d1ce4e5b9n)&mask;z=((z^(z>>27n))*0x94d049bb133111ebn)&mask;z=(z^(z>>31n))&mask;return z%SCALE}}
  class FixedLedger{
    ...
    deltaSigned(name,value){const delta=q(value);this.signed.set(name,this.getSigned(name)+delta);return delta}
    ...
    snapshot(){const digest=this.digest();return{seed:this.seed,tick:this.tick,microstep:this.microstep,accounts:digest.bounded,signed:digest.signed,audit:this.audit.slice(-64)}}
