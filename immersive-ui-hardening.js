...
(()=>{
'use strict';
let restoreFocus=null,heartGesture=null,lastCrisisOpen=false,selectionReady=false;
...
function closeCrisis(reason='dismissed'){const c=crisis();if(!c)return;c.classList.remove('open');c.setAttribute('aria-hidden','true');document.dispatchEvent(new CustomEvent('cb:immersive-crisis-closed',{detail:{reason}}));document.querySelector('.ci-end')?.focus()}
...
function syncDock(){
 ...
 document.querySelectorAll('.ci-ability[data-agent]').forEach(b=>{
  ...
  const selectable=selectionReady&&!!g.running&&(Number(g.moves??0)>0)&&(Number(g.energy??0)>=cost);
  b.disabled=!selectable;b.setAttribute('aria-disabled',String(!selectable));
  b.title=!selectionReady?'SELECT A HEART REGION FIRST':!g.running?'RUN COMPLETE':Number(g.moves??0)<1?'MOVE ALREADY USED':Number(g.energy??0)<cost?'INSUFFICIENT ENERGY':`Cost ${cost} energy`;
 });
...
function suppressDragSelection(){
 ...
 host.addEventListener('pointermove',e=>{if(!heartGesture)return;if(Math.hypot(e.clientX-heartGesture.x,e.clientY-heartGesture.y)>8){heartGesture.moved=true;window.__cbHeartWasDragging=true}},true);
 ...
 host.addEventListener('pointercancel',e=>{if(heartGesture){e.stopImmediatePropagation();e.stopPropagation()}heartGesture=null;window.__cbHeartWasDragging=true},true);
 host.addEventListener('click',e=>{if(window.__cbHeartWasDragging){e.stopImmediatePropagation();e.preventDefault();window.__cbHeartWasDragging=false}},true);
}
function bind(){
 ...
 document.addEventListener('click',e=>{const t=e.target;if(t.closest('#ciCrisis [data-agent]'))document.dispatchEvent(new CustomEvent('cb:immersive-crisis-closed',{detail:{reason:'choice-commit'}}));else if(t.closest('#ciCrisis [data-close]'))document.dispatchEvent(new CustomEvent('cb:immersive-crisis-closed',{detail:{reason:'dismissed'}}))},true);
 ...
 const c=crisis();if(c)new MutationObserver(()=>{const isOpen=c.classList.contains('open');if(isOpen&&!lastCrisisOpen)requestAnimationFrame(()=>focusables(c)[0]?.focus());lastCrisisOpen=isOpen}).observe(c,{attributes:true,attributeFilter:['class']});
 document.addEventListener('cb:immersive-crisis',()=>requestAnimationFrame(()=>focusables(crisis()||document).find(Boolean)?.focus()));
 document.addEventListener('cb:immersive-selection',()=>{selectionReady=true;syncDock()});
 app()?.on?.('new-run',()=>{selectionReady=false;window.CBImmersiveSelection=null;closeCrisis('new-run');syncDock()});
 app()?.on?.('load',()=>{selectionReady=true;syncDock()});
...
window.CBImmersiveUIHardening={closePanel,closeCrisis,syncDock,get selectionReady(){return selectionReady}};
})();