...
export class CardiacAppController{
  ...
  constructor({engine,physiology,documentRef=document}){if(!engine)throw new Error('engine required');if(!physiology)throw new Error('physiology required');this.#engine=engine;this.#physiology=physiology;this.#document=documentRef;this.#render=new RenderScheduler()}
  mount(){if(this.#started)return this;this.#bindControls();this.#loadOrCreate();this.#started=true;this.#emit('mounted');return this}
  get state(){return this.#game} get selectedCell(){return this.#selectedCell} get selectedRegion(){return this.#game?.cells?.[this.#selectedCell]||null} get engine(){return this.#engine} get performance(){return this.#performance.snapshot()}
  ...
  advanceDay(){if(!this.#game)this.newRun();const result=this.#performance.measure('turn.resolve',()=>{const scenario=this.#engine.scenarios[this.#game.scenario];const physiologyResult=this.#physiology.phase?.(this.#game,scenario);if(physiologyResult?.hidden)this.#game.hidden=physiologyResult.hidden;return this.#engine.resolve(this.#game)});this.#emit('resolved',result);this.#render.schedule(()=>this.#requestLegacyRefresh());return result}
  ...
  #loadOrCreate(){if(!this.load())this.newRun(this.#scenario())} #scenario(){return this.#document.getElementById('scenario')?.value||'ischemia'}
  #bindControls(){
    const capture=(handler)=>(event)=>{event.preventDefault();event.stopImmediatePropagation();handler(event)};
    this.#document.getElementById('newRun')?.addEventListener('click',capture(()=>this.newRun(this.#scenario())),true);
    this.#document.getElementById('nextDay')?.addEventListener('click',capture(()=>this.advanceDay()),true);
    this.#document.getElementById('scenario')?.addEventListener('change',capture(e=>this.newRun(e.target.value)),true);
    const tissue=this.#document.getElementById('tissue');
    tissue?.addEventListener('click',event=>{const rect=tissue.getBoundingClientRect();const x=Math.floor(((event.clientX-rect.left)/rect.width)*this.#engine.W);const y=Math.floor(((event.clientY-rect.top)/rect.height)*this.#engine.H);this.selectCell(y*this.#engine.W+x)},true);
    this.#document.addEventListener('keydown',event=>{if(event.target instanceof HTMLElement&&['INPUT','SELECT','TEXTAREA','BUTTON'].includes(event.target.tagName))return;const cell=this.#selectedCell;const delta=event.key==='ArrowLeft'?-1:event.key==='ArrowRight'?1:event.key==='ArrowUp'?-this.#engine.W:event.key==='ArrowDown'?this.#engine.W:null;if(delta!==null){event.preventDefault();this.selectCell(Math.max(0,Math.min(this.#engine.W*this.#engine.H-1,cell+delta)))}});
  }
