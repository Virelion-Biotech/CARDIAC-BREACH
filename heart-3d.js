...
function fallback(){
 ...
 el.innerHTML='<div class="heart3d-fallback"><div class="fallback-heart">❤</div><div><b>3D CARDIAC MODEL</b><span>Interactive model unavailable — tactical simulation remains fully playable.</span></div></div>';
}
function clamp(v,a,b){return Math.max(a,Math.min(b,v));}
function loadScript(){return import(CDN);}
function roundedBox(THREE,w,h,d,r,mat){
 const g=new THREE.BoxGeometry(w,h,d,6,6,6); return new THREE.Mesh(g,mat);
}
function sphere(THREE,rx,ry,rz,mat){
 const g=new THREE.SphereGeometry(1,48,32);const m=new THREE.Mesh(g,mat);m.scale.set(rx,ry,rz);return m;
}
function tube(THREE,pts,radius,mat){
 const curve=new THREE.CatmullRomCurve3(pts);const g=new THREE.TubeGeometry(curve,32,radius,16,false);return new THREE.Mesh(g,mat);
}
async function init(){
 const el=host(); if(!el)return;
 try{
  ...
  scene.fog=new THREE.Fog(0x071018,10,24);
  const camera=new THREE.PerspectiveCamera(32,width()/height(),0.1,100); camera.position.set(0.15,0.15,8.3);
  const renderer=new THREE.WebGLRenderer({antialias:true,alpha:true,powerPreference:'high-performance'});
  renderer.setPixelRatio(Math.min(devicePixelRatio,2)); renderer.setSize(width(),height(),false); renderer.outputColorSpace=THREE.SRGBColorSpace;
  ...
  const root=new THREE.Group(); root.rotation.set(-0.10,0.16,0); scene.add(root);
  const ambient=new THREE.HemisphereLight(0x86c8ff,0x091014,2.1); scene.add(ambient);
  const key=new THREE.DirectionalLight(0xffe4d2,3.5); key.position.set(4,6,8); scene.add(key);
  const rim=new THREE.PointLight(0x62f1d7,32,14); rim.position.set(-4,1,4); scene.add(rim);
  const accent=new THREE.PointLight(0xff5d71,18,12); accent.position.set(4,-2,3); scene.add(accent);
  ...
  const cyanMat=new THREE.MeshBasicMaterial({color:0x56e6d0});
  const goldMat=new THREE.MeshBasicMaterial({color:0xd9ff62});
  const darkMat=new THREE.MeshBasicMaterial({color:0x0a1117,transparent:true,opacity:.5,wireframe:true});
  const structure=new THREE.Group(); root.add(structure);

  // Muscular core: layered lobes to create a recognisable heart silhouette.
  ...
  const septum=sphere(THREE,.68,1.34,1.16,new THREE.MeshPhysicalMaterial({color:0x7d2639,roughness:.5})); septum.position.set(.02,.1,.25);
  [left,right,apex,septum].forEach(m=>structure.add(m));
  ...
  // Faint wire shell gives the model a scientific/instrumented look.
  const shell=sphere(THREE,1.98,2.35,1.48,darkMat); shell.position.set(0,.18,0); shell.scale.set(.98,1.02,1); root.add(shell);

  const labels=new THREE.Group(); root.add(labels);
  function label(text,color='white',small=false){
    const c=document.createElement('canvas'); c.width=small?256:384;c.height=72;const x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);x.font=`800 ${small?22:28}px Inter,Arial,sans-serif`;x.fillStyle=color;x.fillText(text,8,40);const tx=new THREE.CanvasTexture(c);const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:tx,transparent:true,depthWrite:false}));sp.scale.set(small?1.55:2.1,.44,1);return sp;
  }
  const lab=label('CARDIAC CORE','#d9ff62');lab.position.set(-2.7,3.15,0); labels.add(lab);
  const mode=label('LIVE MODEL','#67ead7',true);mode.position.set(2.0,-2.9,0);labels.add(mode);

  const hotspot=sphere(THREE,.20,.20,.20,new THREE.MeshBasicMaterial({color:0xff5d71}));hotspot.position.set(0,-.1,1.34);structure.add(hotspot);
  const ring=new THREE.Mesh(new THREE.TorusGeometry(.34,.035,10,48),new THREE.MeshBasicMaterial({color:0xd9ff62,transparent:true,opacity:.9}));ring.position.copy(hotspot.position);ring.rotation.x=Math.PI/2;structure.add(ring);

  let dragging=false,lastX=0,lastY=0,zoom=1,hovered=null;
  const ray=new THREE.Raycaster(),pointer=new THREE.Vector2();
  const pickables=[left,right,apex,septum,aorta,pulmonary];
  ...
  renderer.domElement.addEventListener('pointermove',e=>{if(dragging){root.rotation.y+=(e.clientX-lastX)*.008;root.rotation.x=clamp(root.rotation.x+(e.clientY-lastY)*.005,-.75,.5);lastX=e.clientX;lastY=e.clientY;}else{point(e);ray.setFromCamera(pointer,camera);const hit=ray.intersectObjects(pickables,false)[0];if(hit!==hovered){if(hovered)hovered.material.emissiveIntensity?.=0;hovered=hit?.object||null;renderer.domElement.style.cursor=hovered?'pointer':'grab';}}});
  renderer.domElement.addEventListener('pointerleave',()=>dragging=false);
  renderer.domElement.addEventListener('wheel',e=>{e.preventDefault();zoom=clamp(zoom+e.deltaY*.0012,.72,1.32);},{passive:false});
  renderer.domElement.addEventListener('dblclick',()=>{root.rotation.set(-.1,.16,0);zoom=1;});

  function resize(){const w=width(),h=height();camera.aspect=w/h;camera.updateProjectionMatrix();renderer.setSize(w,h,false);}
  ...
  api={scene,root,hotspot,ring,renderer,setStress(v){accent.intensity=12+v*30;hotspot.material.color.set(v>.65?0xff5d71:0xd9ff62);}};
  window.CB_Heart3D=api;
  document.addEventListener('cardiac:heart-state',e=>{const v=clamp(Number(e.detail?.severity||0),0,1);api.setStress(v);});
