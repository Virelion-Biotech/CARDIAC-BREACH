 ...
 * Non-invasive gameplay extensions: contracts, autosave, achievements,
 * run diagnostics, keyboard controls, accessibility, and scenario modifiers.
 */
(function(){
  'use strict';
  const KEY='cb_advanced_v1';
  ...
  let unlocked=get('achievements',[]);
  let modifier=get('modifier','standard');
  let notes=get('notes','');
  const modifiers={
    standard:{name:'STANDARD',desc:'Baseline simulation conditions.',energy:0,damage:0},
    scarcity:{name:'ENERGY SCARCITY',desc:'Tighter energy economy. Rewards efficient teams.',energy:-18,damage:0},
    volatility:{name:'HIGH VOLATILITY',desc:'State changes are less predictable. Rewards adaptation.',energy:0,damage:.10},
    no_regen:{name:'NO REGENERATION',desc:'Regenerator policies are unavailable. Test alternative strategies.',energy:0,damage:.05}
  };
  const contracts=[
  ...
  function style(){
    if($('advancedStyle'))return;
    const s=document.createElement('style');s.id='advancedStyle';s.textContent=`
      .advanced-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.advanced-card{background:rgba(8,18,22,.72);border:1px solid rgba(117,230,208,.16);padding:12px;border-radius:8px}.advanced-card h3{margin:0 0 6px;font-size:11px;letter-spacing:.14em}.advanced-card p{margin:0;color:#91a7ad;font-size:12px;line-height:1.5}.contract{padding:9px 0;border-top:1px solid rgba(255,255,255,.06)}.contract:first-child{border-top:0}.contract strong{font-size:12px}.contract small{display:block;color:#7f969d;margin-top:3px}.contract.done strong{color:#d9ff62}.badge{display:inline-flex;gap:5px;padding:4px 7px;margin:3px;border:1px solid rgba(217,255,98,.2);border-radius:99px;font-size:10px}.advanced-select,.advanced-note{width:100%;box-sizing:border-box;background:#071216;color:#d8e6e9;border:1px solid rgba(255,255,255,.12);padding:8px;border-radius:5px}.advanced-note{min-height:70px;resize:vertical}.kbd{border:1px solid rgba(255,255,255,.2);padding:1px 5px;border-radius:3px;font-size:10px}.diagnostic-row{display:flex;justify-content:space-between;padding:5px 0;font-size:11px}.risk-high{color:#ef6969}.risk-mid{color:#f1bd62}.risk-low{color:#d9ff62}@media(max-width:760px){.advanced-grid{grid-template-columns:1fr}}
    `;document.head.appendChild(s);
  }
  function panel(){
    if($('advancedPanel'))return;
    const wrap=document.createElement('section');wrap.className='lower';wrap.innerHTML=`
      <div class="panel" id="advancedPanel"><div class="panel-head"><span>COMMAND DECK</span><span id="advStatus">AUTOSAVE ON</span></div>
  ...
  function risk(){
    const vals=[['VIABILITY',state.viability,false],['FUNCTION',state.func,false],['INFLAMMATION',state.inflammation,true],['FIBROSIS',state.fibrosis,true],['OXYGEN',state.oxygen,false],['ARRHYTHMIA',state.arrhythmia,true],['ENERGY',energy,false]];
    return vals.map(([n,v,bad])=>{const r=bad?v:100-v;const cls=r>65?'risk-high':r>35?'risk-mid':'risk-low';return `<div class="diagnostic-row"><span>${n}</span><b class="${cls}">${Math.round(v)}%</b></div>`}).join('');
  }
  function renderCareer(){
    if(!$('careerStats'))return;const pts=stats.wins*100+stats.contracts*20+stats.perfect*50; $('careerScore').textContent=pts;
    $('careerStats').innerHTML=`Runs <b>${stats.runs}</b> · Completed <b>${stats.wins}</b> · Best score <b>${stats.best}</b> · Days simulated <b>${stats.days}</b> · Contracts <b>${stats.contracts}</b>`;
    $('badges').innerHTML=unlocked.length?unlocked.map(x=>`<span class="badge">◆ ${x}</span>`).join(''):'<span class="muted">No achievements yet.</span>';
  }
  function autosave(){
    if(typeof day==='undefined')return;
    const snap={day,energy,agents:JSON.parse(JSON.stringify(agents||[])),state:JSON.parse(JSON.stringify(state)),history:JSON.parse(JSON.stringify(history||[])),scenario:$('scenario')?.value||'ischemia',modifier,notes};
    put('save',snap);
  }
  function resume(){
    const s=get('save',null);if(!s)return log('NO AUTOSAVE FOUND.',true);
  ...
  function finish(){
    ...
    stats.runs++;stats.wins++;stats.days+=day;const sc=score();stats.best=Math.max(stats.best,sc);
    const passed=activeContracts.filter(c=>c.check());stats.contracts+=passed.length;if(sc>=90)stats.perfect++;if(energy>=35)stats.efficiency++;
    const unlock=(n)=>{if(!unlocked.includes(n)){unlocked.push(n);log('ACHIEVEMENT UNLOCKED — '+n)}};
    if(sc>=85)unlock('High Performer');if(energy>=50)unlock('Resource Steward');if(passed.length===3)unlock('Contract Master');if(state.viability>=80&&state.func>=80)unlock('Tissue Guardian');if(day<=MAX_DAYS&&sc>=80)unlock('Consistent Operator');
    put('stats',stats);put('achievements',unlocked);autosave();renderCareer();
  }
  function hook(){
    const newRun=$('newRun'),next=$('nextDay');
    ...
    if(next)next.addEventListener('click',()=>setTimeout(()=>{if(day>0&&day<MAX_DAYS)autosave();if(day===MAX_DAYS)finish();renderContracts();if($('riskRadar'))$('riskRadar').innerHTML=risk();},40));
