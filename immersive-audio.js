...
function osc(freq,dur,type='sine',vol=.05,delay=0,slide=0){const c=ensure();if(!c||!enabled)return;const t=c.currentTime+delay,o=c.createOscillator(),g=c.createGain();o.type=type;o.frequency.setValueAtTime(freq,t);if(slide)o.frequency.exponentialRampToValueAtTime(Math.max(35,freq+slide),t+dur);g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(vol,t+.008);g.gain.exponentialRampToValueAtTime(.0001,t+dur);o.connect(g);g.connect(master);o.start(t);o.stop(t+dur+.02)}
...
function setVolume(v){volume=Math.max(0,Math.min(1,Number(v)||0));const c=ensure();if(master&&c)master.gain.setTargetAtTime(enabled?volume*.18:0,c.currentTime,.03);persist();updateButton()}
...
function bind(){
 ...
 if(panel)new MutationObserver(()=>{const isOpen=panel.classList.contains('open');if(isOpen&&!lastPanel)panelOpen();if(!isOpen&&lastPanel)panelClose();lastPanel=isOpen?'1':''}).observe(panel,{attributes:true,attributeFilter:['class']});
 ...
 window.CBApp?.on?.('resolved',e=>e?.payload?.ok===false?fail():resolve());
 addControl();
}
